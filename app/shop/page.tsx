...
import { ShopDocumentData } from "@/prismicio-types";

export default async function Shop() {
  const client = createClient();
  const shopPageResponse = await client.getSingle("shop", { lang: "fr-fr" });
  const { data }: { data: ShopDocumentData } = shopPageResponse;

  const productsResponse = await client.getAllByType("product", {
    lang: "fr-fr",
  });
